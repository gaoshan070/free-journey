...
if(!class_exists('FPD_Scripts_Styles')) {
	...
	class FPD_Scripts_Styles {
		...
		public function register() {
			...
			if($local_test) {
				$timestamp = time();
				wp_enqueue_style( 'fpd-test-webfont', 'http://radykal.dev/fpd/src/FontFPD/style.css', false, Fancy_Product_Designer::VERSION );
				wp_enqueue_style( 'fpd-test-plugins', 'http://radykal.dev/fpd/dist/css/plugins.min.css', false, Fancy_Product_Designer::FPD_VERSION );
			}
			...
			wp_register_script( 'fabric', plugins_url('/assets/js/'.$fabricjs_file, FPD_PLUGIN_ROOT_PHP), false, '1.6.1' );

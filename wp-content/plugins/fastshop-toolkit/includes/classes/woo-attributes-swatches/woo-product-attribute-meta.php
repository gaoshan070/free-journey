<?php
if ( !class_exists( 'Fastshop_Attribute_Product_Meta' ) ) {
	class Fastshop_Attribute_Product_Meta
	{
		public $screen;
		public $taxonomy;
		public $meta_key;
		 ...
		 *
		 * @return Fastshop_Attribute_Product_Meta
		 */
		...
		{
			if ( is_product() ) {
				wp_enqueue_style( 'woo-attributes-swatches-css', FASTSHOP_TOOLKIT_URL . 'includes/classes/woo-attributes-swatches/woo-attribute.css', array(), '1.0' );
				wp_enqueue_script( 'woo-attributes-swatches-js', FASTSHOP_TOOLKIT_URL . 'includes/classes/woo-attributes-swatches/woo-attribute.js', array(), '1.0' );
			}
		...
		{
			$kuteshop_types = array(
				'color' => esc_html__( 'Box style', 'fastshop' ),
			);

			return array_merge( $types, $kuteshop_types );
		}
		...
		public function kt_woocommerce_product_option_terms( $attribute_taxonomy, $i )
		{
            ...
            <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'fastshop' ); ?>"
            ...
            <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'fastshop' ); ?></button>
            <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'fastshop' ); ?></button>
            <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'fastshop' ); ?></button>
		<?php endif; ?>
			<?php
		}
		...
		public function woocommerce_add_attribute_thumbnail_field()
		{
            ...
            <div class="form-field ">
                <label for="product_attribute_type_<?php echo $this->meta_key; ?>"><?php esc_html_e( 'Type', 'fastshop' ) ?></label>
                <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                    ...
                    <option value="-1"><?php esc_html_e( 'None', 'fastshop' ) ?></option>
                    <option value="color"><?php esc_html_e( 'Color', 'fastshop' ) ?></option>
                    <option value="photo"><?php esc_html_e( 'Photo', 'fastshop' ) ?></option>
                    <option value="label"><?php esc_html_e( 'Label', 'fastshop' ) ?></option>
                </select>
            ...
            <div class="form-field swatch-field field-color section-color-swatch"
                ...
                <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                    <label><?php esc_html_e( 'Color', 'fastshop' ); ?></label>
            ...
            <div class="form-field swatch-field field-photo" style="overflow:visible;display:none;">
                <div id="swatch-photo" class="<?php echo sanitize_title( $this->meta_key ); ?>-photo">
                    <label><?php esc_html_e( 'Thumbnail', 'fastshop' ); ?></label>
                    ...
                    <div style="line-height:60px;">
                        ...
                        <button type="submit"
                                class="upload_image_button button"><?php esc_html_e( 'Upload/Add image', 'fastshop' ); ?></button>
                        <button type="submit"
                                class="remove_image_button button"><?php esc_html_e( 'Remove image', 'fastshop' ); ?></button>
                    </div>
                    <script type="text/javascript">
		...
		{
			global $woocommerce;
			$swatch_term = new Fastshop_Term( $this->meta_key, $term->term_id, $taxonomy, false, $this->image_size );
			$image       = '';
			?>
            <tr class="form-field ">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Type', 'fastshop' ); ?></label></th>
                <td>
                    <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]"
                            id="product_attribute_swatchtype_<?php echo $this->meta_key; ?>" class="postform">
                        <option <?php selected( 'none', $swatch_term->get_type() ); ?>
                                value="-1"><?php esc_html_e( 'None', 'fastshop' ); ?></option>
                        <option <?php selected( 'color', $swatch_term->get_type() ); ?>
                                value="color"><?php esc_html_e( 'Color', 'fastshop' ); ?></option>
                        <option <?php selected( 'photo', $swatch_term->get_type() ); ?>
                                value="photo"><?php esc_html_e( 'Photo', 'fastshop' ); ?></option>
                        <option <?php selected( 'label', $swatch_term->get_type() ); ?>
                                value="label"><?php esc_html_e( 'Label', 'fastshop' ); ?></option>
                    </select>
                    <script type="text/javascript">
                        jQuery(document).ready(function ($) {
            ...
            <tr class="form-field swatch-field swatch-field-color section-color-swatch"
                ...
                <th scope="row" valign="top"><label><?php esc_html_e( 'Color', 'fastshop' ); ?></label></th>
                <td>
                    <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
                        <div id="product_attribute_color_<?php echo $this->meta_key; ?>_picker" class="colorSelector">
                            <div></div>
            ...
            <tr class="form-field swatch-field swatch-field-photo" style="overflow:visible;<?php echo $style; ?>">
                <th scope="row" valign="top"><label><?php esc_html_e( 'Photo', 'fastshop' ); ?></label></th>
                <td>
                    <div id="product_attribute_thumbnail_<?php echo $this->meta_key; ?>"
                         style="float:left;margin-right:10px;">
		...
		public function woocommerce_product_attribute_columns( $columns )
		{
			...
			$new_columns[ $this->meta_key ] = esc_html__( 'Thumbnail', 'fastshop' );
			unset( $columns[ 'cb' ] );
		...
		public function woocommerce_product_attribute_column( $columns, $column, $id )
		{
			if ( $column == $this->meta_key ) :
				$swatch_term = new Fastshop_Term( $this->meta_key, $id, $this->taxonomy, false, $this->image_size );
		...
		/**
		 * Override WooCommerce function kuteshop_variation_attribute_options (Locate in wc-template-functions.php)
		...
		public function wc_variation_attribute_options( $html, $args )
		{
			...
			$args                    = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
					...
					'id'               => '',
					'class'            => '',
					'show_option_none' => __( 'Choose an option', 'fastshop' ),
				)
			);
			...
			$show_option_none_text = $args[ 'show_option_none' ] ? $args[ 'show_option_none' ] : __( 'Choose an option', 'fastshop' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
			...
			if ( !empty( $options ) ) {
				if ( $product && taxonomy_exists( $attribute ) ) {
					...
					$html               = '<select data-attributetype="' . $attribute_taxonomy[ 'type' ] . '" id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '" name="' . esc_attr( $name ) . '" data-attribute_name="attribute_' . esc_attr( sanitize_title( $attribute ) ) . '" data-show_option_none="' . ( $show_option_none ? 'yes' : 'no' ) . '">';
					...
					foreach ( $terms as $term ) {
						if ( in_array( $term->slug, $options ) ) {
							...
							if ( function_exists( 'fastshop_resize_image' ) ) {
								$image_thumb = fastshop_resize_image( $data_photo, null, $attribute_swatch_width, $attribute_swatch_height, true, true, false );
		...
		public function get_product_attribute( $attribute )
		{
			...
			try {
				...
				if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
					throw new WC_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'fastshop' ), 404 );
				}
...
new Fastshop_Attribute_Product_Meta();
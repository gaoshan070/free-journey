<?php
if ( !class_exists( 'Fastshop_Shortcode' ) ) {
	class Fastshop_Shortcode
	{
		/**
		 * Shortcode name.
		 *
		 ...
		 * @var  string
		 */
		protected $metakey = '_Fastshop_Shortcode_custom_css';

		public function __construct()
		{
			if ( !empty( $this->shortcode ) ) {
				// Add shortcode.
				add_shortcode( "fastshop_{$this->shortcode}", array( &$this, 'output_html' ) );
		...
		public function replace_post( $post_id )
		{
			...
			if ( $post ) {
				if ( has_shortcode( $post->post_content, "fastshop_{$this->shortcode}" ) ) {

					$post->post_content = preg_replace_callback(
						'/(' . $this->shortcode . '_custom_id)="[^"]+"/',
						'Fastshop_Shortcode_replace_post_callback',
						$post->post_content
					);
		...
		public function get_css( $content )
		{
			...
			if ( preg_match_all( '/' . get_shortcode_regex() . '/', $content, $shortcodes ) ) {
				foreach ( $shortcodes[ 2 ] as $index => $tag ) {
					if ( strpos( $tag, 'fastshop_' ) !== false ) {
						$atts      = shortcode_parse_atts( trim( $shortcodes[ 3 ][ $index ] ) );
						$shortcode = explode( '_', $tag );
						$shortcode = end( $shortcode );

						$class = 'Fastshop_Shortcode_' . implode( '_', array_map( 'ucfirst', explode( '-', $shortcode ) ) );
		...
		public function getProducts( $atts, $args = array(), $ignore_sticky_posts = 1 )
		{
			...
			switch ( $target ):
				...
				case 'product_attribute' :
					...
					break;
				case 'on_new' :
					$newness = fastshop_get_option( 'product_newness', 7 );    // Newness in days as defined by option
...
if ( !function_exists( 'Fastshop_Shortcode_replace_post_callback' ) ) {

	function Fastshop_Shortcode_replace_post_callback( $matches )
	{
		// Generate a random string to use as element ID.
		$id = 'fastshop_custom_css_' . mt_rand();
...
foreach ( $shortcodes as $shortcode ) {
	...
	if ( is_file( FASTSHOP_TOOLKIT_PATH . '/includes/shortcodes/' . $shortcode . '.php' ) ) {

		include_once FASTSHOP_TOOLKIT_PATH . '/includes/shortcodes/' . $shortcode . '.php';
	}
...
if ( !function_exists( 'Fastshop_Shortcode_print_inline_css' ) ) {
	function Fastshop_Shortcode_print_inline_css()
	{
		// Get all custom inline CSS.
		if ( is_singular() ) {
			$post_custom_css = get_post_meta( get_the_ID(), '_Fastshop_Shortcode_custom_css', true );
			$inline_css[]    = $post_custom_css;
			$inline_css      = apply_filters( 'fastshop-shortcode-inline-css', $inline_css );
			if ( count( $inline_css ) ) {
				wp_add_inline_style( 'fastshop_custom_css', implode( ' ', $inline_css ) );
			}
		}
	}
}
add_action( 'wp_enqueue_scripts', 'Fastshop_Shortcode_print_inline_css', 999 );
...
//			'success' => 'no',
//		);
//		$shortcode = isset( $_POST[ 'atts' ] ) ? $_POST[ 'atts' ] : '';

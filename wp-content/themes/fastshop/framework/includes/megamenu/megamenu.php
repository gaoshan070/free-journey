...
if ( !class_exists( 'Fastshop_MegaMenu' ) ) {
	class Fastshop_MegaMenu
	{
		...
		function register_scripts( $hook )
		{
			...
			wp_enqueue_style( 'fastshop-megamenu', get_theme_file_uri( '/framework/includes/megamenu/css/megamenu.css' ) );
			wp_enqueue_style( 'magnific-popup', get_theme_file_uri( '/framework/includes/megamenu/css/magnific-popup.min.css' ) );
			wp_enqueue_style( 'font-awesome', get_theme_file_uri( 'assets/css/font-awesome.min.css' ), array(), '2.4' );
			wp_enqueue_media();
			wp_enqueue_script( 'fastshop-megamenu', get_theme_file_uri( '/framework/includes/megamenu/js/megamenu.js' ), array( 'jquery' ), '1.0.0', true );
			wp_enqueue_script( 'magnific-popup', get_theme_file_uri( '/framework/includes/megamenu/js/jquery.magnific-popup.min.js' ), array( 'jquery' ), '1.0.0', true );

			wp_localize_script( 'fastshop-megamenu', 'fastshop_ajax_backend', array(
					'ajaxurl'  => admin_url( 'admin-ajax.php' ),
					'security' => wp_create_nonce( 'fastshop_ajax_backend' ),
				)
			);
		}
            ...
            <div class="icons-popup">
                <div class="inner">
					<?php if ( count( $this->lits_fonts ) > 0 ): ?>
						...
						<?php endforeach; ?>
					<?php else: ?>
						<?php esc_html_e( 'No Font', 'fastshop' ); ?>
					<?php endif; ?>
                </div>
            </div>
			<?php
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				if ( !empty( $shortcodes_custom_css ) ) {
					...
					wp_add_inline_style( 'fastshop_custom_css', $shortcodes_custom_css );
				}
			}
		}

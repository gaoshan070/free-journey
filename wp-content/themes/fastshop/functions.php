...
if ( !class_exists( 'Fastshop_Functions' ) ) {
	class Fastshop_Functions
	{
		...
		{
			add_action( 'after_setup_theme', array( $this, 'fastshop_settup' ) );
		...
		public function fastshop_settup()
		{
			/*
		...
		function scripts()
		{
			...
			// Load fonts
			wp_enqueue_style( 'fastshop-googlefonts', $this->fastshop_fonts_url(), array(), null );
			...
			wp_enqueue_script( 'fastshop-init-script', get_theme_file_uri( '/assets/js/init-script.js' ), array(), '1.0' );

			$gmap_api_key = fastshop_get_option( 'gmap_api_key', '' );
			$gmap_api_key = trim( $gmap_api_key );
			wp_enqueue_script( 'maps', esc_url( 'https://maps.googleapis.com/maps/api/js?key=' . esc_attr( $gmap_api_key ) ), array( 'jquery' ), null, true );

			wp_enqueue_script( 'fastshop-script', get_theme_file_uri( '/assets/js/functions.min.js' ), array(), '1.0' );

			if ( is_rtl() ) {
				wp_enqueue_script( 'fastshop-rtl', get_theme_file_uri( '/assets/js/rtl.js' ), array(), '1.0' );
			}
